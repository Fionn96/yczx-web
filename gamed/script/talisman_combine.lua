...
function ZLuaTalisman_Combine(id1,id2,talisman1,talisman2,cid,output,locked1,locked2)
	...
	---	output:CreateItem(-1)
  ---元宵灯不允许血炼
  if id1 == talismans["US_45_YuanXiaoDeng"].tmplID or id2 == talismans["US_45_YuanXiaoDeng"].tmplID then
		output:CreateItem(-1)
...
---//旧太一1-6技能概率
		local skillnumchanceseq={0,0,0,0,0,1}
		if cid == 23882 then skillnumchanceseq={0,0,0,0,0,1} end
		---//礼包太一1-6技能调整概率
		if cid == 62752 or cid == 69513 then skillnumchanceseq={0,0,0,0,0,1} end
		---//新太一1-6技能概率
		if cid == 62754 then skillnumchanceseq={0,0,0,0,0,1} end
		---//单独洗格数道具概率
		local HowManySkills
...
function ZLuaTalisman_ChooseTalismanSkill(TalismanSkillID,cid)
  ...
  --	TalismanSkillIDs[3088]={3088,3088,3088}---//元宵灯祝_元宵灯
	...
	local skilllvlchanceseq={0,0,1}
	if cid == 23882 then skilllvlchanceseq={0,0,1} end  ---//礼包
	if cid == 62752 or cid == 69513 then skilllvlchanceseq={0,0,1} end    ---//新
	if cid == 62753 then 
...
--		return false
	...
	elseif current_lvl > 1 then
		if cid == 79530 and current_lvl <= 8 then
			current = current;
		else
			current= current - 1
		end
